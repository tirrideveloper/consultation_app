...
import 'package:consultation_app/notification_handler.dart';
...
class FirebaseNotifications {
  ...
  void firebaseCloudMessageListener(BuildContext context) async {
    NotificationSettings setting = await _messaging.requestPermission(
        ...
        provisional: true,
        sound: true);
    print("AYAR ${setting.authorizationStatus}");
    _messaging.getToken().then((token) => print("BENİM TOKEN: $token"));

    _messaging
        .subscribeToTopic("demo")
        .whenComplete(() => print("TAMAMDIR YAA"));

    FirebaseMessaging.onMessage.listen(
      (remoteMessage) {
        print("ANAZININ AMCUGU $remoteMessage");
    ...
    FirebaseMessaging.onMessageOpenedApp.listen(
      (remoteMessage) {
        print("ANNESİZ open app: $remoteMessage");
        if (Platform.isIOS)
          showDialog(
            context: myContext,
  ...
  static void showNotification(String title, String body) async {
    ...
    await NotificationHandler.flutterNotificationHandlerPlugin
        .show(Random().nextInt(1000), title, body, platForm, payload: "Benim Payload");
  }
}
